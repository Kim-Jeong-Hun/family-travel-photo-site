...
function PhotoSection({ imageUrls = [], content }) { // 기본값 [] 설정으로 undefined 방지
      ...
      */}
      ...
      {selectedIndex !== null && displayImages[selectedIndex] && (
        <div
          ...
          onClick={closeModal}
        >
          <button className="absolute top-6 right-6 text-white text-4xl z-[70]" onClick={closeModal}>&times;</button>
          
          <div className="relative bg-white w-full max-w-[550px] flex flex-col rounded-sm overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="relative aspect-square w-full">
              ...
              <div className="absolute inset-0 flex justify-between items-center px-2 pointer-events-none">
                {selectedIndex > 0 && (
                  <button className="w-10 h-10 bg-black/30 hover:bg-black/50 text-white rounded-full flex items-center justify-center transition-colors pointer-events-auto" onClick={() => setSelectedIndex(selectedIndex - 1)}>‹</button>
                )}
            ...
            <div className="p-6 h-[120px] lg:h-[160px] flex flex-col justify-start">
              <p className="text-black font-bold text-[17px] line-clamp-3">{content}</p>
            </div>
          </div>
        </div>
      )}
